...
void setup() {
  ...
  debugTransS = SimpleTransSender(&radio, matchOut[0], matchOut[1], 100);
  // ledSerialR = SimpleTransReceiver(matchIn[0], matchIn[1], 101);
...
void loop() {
  ...
  if((millis()-lastMillis)>1000){
    ...
    lastMillis = millis();
    debugTransS.sendFloat(lastMillis);
  }
  // ------------ SERIAL IN ------------- //
...
//   if (Serial.available()) {
//     Serial.readBytes((char*)inBytes,5);
//     return;
//   }
// }
void getTransceiver(byte* inBytesTrans){
  ...
  if (radio.available()) {
    radio.read((char*)inBytesTrans,5);
  }
  radio.stopListening();
}
void clearInput() {
